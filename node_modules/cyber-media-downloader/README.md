<h3 align="center">
  ...
  <p align="center"><img src="https://img.shields.io/badge/WLCM%20TO  MEDIA DOWNLOADER-green?colorA=%23ff0000&colorB=%23017e40&style=flat-square">  
  
</h3>

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Neuton&size=25&color=30FF40&background=000000&center=true&vCenter=true&width=360&height=60&lines=Hello+World%2C+I'm+CYBER-DOWNLOAR+Here+🤙;𝙸𝚃'𝚜+𝙽𝙾𝚃+𝙰+𝙹𝚄𝚂𝚃+𝙽𝙰𝙼𝙴+𝙱𝚁𝙾+🥱;𝙸𝚃'𝚜+𝙰+𝙱𝚁𝙰𝙽𝙳+🔥;Respect+Mr.NAYAN+🥀;Thanks+My+All+Friend+🤙+🥰)](https://git.io/typing-svg)



## Instalation :
```bash
> npm i cyber-media-downloader
```
## 🚀 Project Update Log  
...
## Example (fb & insta)
```js
const { ndown } = require("cyber-media-downloader")
...
## Instagram Usage 
```js
const {instagram} = require("cyber-media-downloader");
...
## Example (TikTok)
```js
const { tikdown } = require("cyber-media-downloader")
...
## Example (YouTube)
```js
const { ytdown } = require("cyber-media-downloader")
let URL = await ytdown("https://youtu.be/aRSuyrZFu_Q?si=bsfzgeeGmRpsHqnF")
console.log(URL)
```
## Example (Twitter)
```js
const { twitterdown } = require("cyber-media-downloader")
...
## Example Fbdown2
```js
const { fbdown2 } = require("cyber-media-downloader");
...
## Usage GDLink 
```js
const {GDLink} = require("cyber-media-downloader");
...
## Usage Pintarest 
```js
const {pintarest} = require("cyber-media-downloader");
...
## Usage CapCut 
```js
const { capcut } = require("cyber-media-downloader");
...
## Usage Likee 
```js
const { likee} = require("cyber-media-downloader");
...
## Usage Threads
```js
const { threads } = require("cyber-media-downloader");
...
```js
(async () => {
  const { spotifySearch } = require("cyber-media-downloader");
...
```js
(async () => {
  const { spotifyDl } = require("cyber-media-downloader");
...
```js
(async () => {
  const { soundcloudSearch } = require("cyber-media-downloaders");
...
```js
(async () => {
  const { soundcloud } = require("cyber-media-downloader");
...
```js
(async () => {
  const { terabox } = require("cyber-media-downloader");
...
```
```js
const {alldown} = require("media-downloaders");
